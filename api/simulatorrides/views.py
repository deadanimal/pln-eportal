...
class SimulatorRideViewSet(NestedViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = SimulatorRideViewSet.objects.all()
        return queryset
...
class SimulatorRideBookingViewSet(NestedViewSetMixin, viewsets.ModelViewSet):
    ...
    def auto_change_status(self, request):
        ...
        for data in queryset:
            ...
            if time_elapsed_second >= 10*60*60:
                simulator_ride = SimulatorRideTime.objects.get(id=data.id)
...
class SimulatorRideTicketViewSet(NestedViewSetMixin, viewsets.ModelViewSet):
    queryset = SimulatorRideTicket.objects.all()
    serializer_class = SimulatorRideTicketSerializer  
    ...
    @action(methods=['GET'], detail=False)
    def display_ticket(self, request):
        simulator_ride_booking_id = request.query_params.get('id', None)
        print("test", simulator_ride_booking_id)
        ticket = SimulatorRideTicket.objects.filter(ticket_booking_id=simulator_ride_booking_id)[0]
        print(ticket)
        html = f'<iframe src={ticket.ticket_link} height="100%" width="100%"></iframe>'
