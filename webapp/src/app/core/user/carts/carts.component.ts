...
export class CartsComponent implements OnInit {
  ...
  submitInvoice() {
    ...
    this.checkVoucherCode();
    console.log("selected carts", this.carts);
  ...
  updateCartStatusToPD() {
    for (let i = 0; i < this.carts.length; i++) {
      ...
      for (let j = 0; j < this.carts[i].show_booking_id.length; j++) {
        if (this.carts[i].show_booking_id[j].ticket_free == true) {
          cart_status = "CM";
          mode = "SHOW";
          user = this.carts[i].show_booking_id[j].user_id;
          full_name = this.carts[i].show_booking_id[j].full_name;

        }
      }

      for (let j = 0; j < this.carts[i].simulator_ride_booking_id.length; j++) {
        if (this.carts[i].simulator_ride_booking_id[j].ticket_free == true) {
          cart_status = "CM";
          mode = "SPACE";
          user = this.carts[i].simulator_ride_booking_id[j].user_id;
          full_name = this.carts[i].simulator_ride_booking_id[j].full_name;

        }
      }

      if (cart_status == "CM" && mode == "SHOW") {
        let notificationBody = {
          title: "Tiket Percuma Tayangan",
          description:
            "Anda telah mendapat tiket percuma. Sila tuntut di kaunter planetarium.",
          user: user.id,
        };
        ...
        let notificationBodyAdmin = {
          title: "Tiket Percuma Tayangan",
          description:
            `${full_name} telah mendapat tiket percuma.`,
          user: user.id,
        };
      ...
      if (cart_status == "CM" && mode == "SPACE") {
        let notificationBody = {
          title: "Tiket Percuma Kembara Simulator",
          description:
            "Anda telah mendapat tiket percuma. Sila tuntut di kaunter planetarium.",
          user: user.id,
        };
