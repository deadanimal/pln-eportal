...
export class FacilitiesApplicationComponent implements OnInit {
    ...
    this.userService.extended("").subscribe(
      (res) => {
        ...
        for (let i = 0; i < res.length; i++) {
          try {
            if (res[i].role.code == "CS") {
              this.users.push(res[i]);
            } else {
              this.pics.push(res[i]);
            }
          }
          catch (err) {}
        }
      ...
      .fire({
        ...
        preConfirm: () => {
          ...
          if (!voucher_code) return true;
            ...
            if (result) {
              this.validated_voucher = { voucher_code: voucher_code, voucher_id: result.id };
      ...
      .then((result) => {
        if (result.isConfirmed == true) {
          ...
          let notificationBody = {
            'title': 'Pengesahan Fasiliti',
            'description': `Permohonan fasiliti ${row.facility_id.name_en} anda telah disahkan`,
            'user': row.user_id.id,
          }

          let notificationBodyAdmin = {
            'title': 'Pengesahan Fasiliti',
            'description': `Permohonan fasiliti ${row.facility_id.name_en} untuk pengguna ${row.user_id.full_name} telah disahkan`,
            'user': row.user_id.id,
          }
          ...
          this.facilitybookingService.update(objUpdate, row.id).subscribe(
            ...
            () => {
              ...
              this.sendNotification(notificationBodyAdmin)
              ...
              //TODO 3 (update voucher status to used)

              // this.updateVoucherAfterUsed();
            }
          );
          ...
          let notificationBody = {
            'title': 'Pengesahan Fasiliti',
            'description': `Permohonan fasiliti ${row.facility_id.name_en} anda telah ditolak`,
            'user': row.user_id.id,
          }

          let notificationBodyAdmin = {
            'title': 'Pengesahan Fasiliti',
            'description': `Permohonan fasiliti ${row.facility_id.name_en} untuk pengguna ${row.user_id.full_name} telah ditolak`,
            'user': row.user_id.id,
          }
  ...
  sendmail(row) {
    console.log("sendmail method invoked", row)
    ...
    if (row.status == "FB01") {
      ...
      obj = {
        ...
        //   voucher_code: this.validated_voucher.voucher_code,
        // }),
        context: {"voucher_code": this.validated_voucher.voucher_code}
      };
      console.log("email send context", obj)
    }
  ...
  updateStatusToFB04(facility_id, user_id) {
    ...
    this.facilitybookingService.update(obj, facility_id).subscribe(
      ...
      () => {
        ...
        //TODO 2 (declare voucher code for email)
        let objMail = {
          status: "FB04",
          user_id: user_id,
        };
  ...
  sendNotification(body) {
    this.notificationService.post(body).subscribe(
      (res) => {
        console.log("res", res);
  ...
  updateVoucherAfterUsed() {
    ...
    this.voucherService.update(body, this.validated_voucher.voucher_id).subscribe(
      (res) => {
        console.log("update voucher success", res)
      },
      (err) => {
