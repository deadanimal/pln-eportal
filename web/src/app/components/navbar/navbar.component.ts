...
export class NavbarComponent implements OnInit {
  ...
  clickForgotPassword() {
    console.log("forgotpasswordformngroup", this.forgotPasswordFormGroup.value);

    this.userService.filter(`email=${this.forgotPasswordFormGroup.value.email}`).subscribe(
      (res) => {
        console.log("res", res)
        if (res.length > 0) {
          this.authService
