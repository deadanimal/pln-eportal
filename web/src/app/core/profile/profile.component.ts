...
export class ProfileComponent implements OnInit {
  ...
  @ViewChild('states_input') myDiv: ElementRef;
  ...
  getDescription(invoicereceipts) {
      ...
      for (let j = 0; j < invoicereceipts[i].cart_id.length; j++) {
        ...
        if (cart_details.show_booking_id.length > 0) {
          for (let k = 0; k < cart_details.show_booking_id.length; k++) {
            ...
            let urlReceipt = invoicereceipts[i].payment_successful_datetime
              ? "<a class='btn btn-sm btn-info btn-icon' href='" +
                environment.baseUrl +
                "v1/invoice-receipts/generate_receipt/?id=" +
                invoicereceipts[i].id +
          ...
          for (
            ...
            console.log(cart_details.simulator_ride_booking_id[k]);
            ...
            let booking_id = simulator_ride_booking.id;
            let urlTicket = invoicereceipts[i].payment_successful_datetime
              ? "<a class='btn btn-sm btn-info btn-icon' href='" +
                environment.baseUrl +
                "v1/simulator-ride-tickets/display_ticket/?id=" +
                booking_id +
                "'  download><i class='fas fa-ticket-alt'></i></a>"
              : "";
    ...
    console.log("invoicereceipts", arrayTransaksiBayaran)
  ...
  ngOnInit() {
    ...
    setInterval(()=> { this.getStates() },  1000);
  ...
  selectTab(tab: string) {
    ...
    setTimeout(() => {
      console.log($("#pages > li "));
      // $("#pages li a").removeAttr("href"));
    }, 1000);
  }
  ...
  getNotificationData() {
    ...
    this.notificationService.filter(`user=${user_id}`).subscribe(
      (res) => {
  ...
  downloadTiketSpace(invoice) {
    console.log(invoice.booking_id);
    this.simulatorBookingService.print_ticket(invoice.booking_id).subscribe(
      (res) => {
        console.log(res);
      },
      (err) => {
  ...
  getStates() {
    ...
    for (let i = 0; i < this.states.length; i++) {
      if (this.states[i].country_id == this.userFormGroup.value.country) {
        this.selected_states.push(this.states[i]);
      }
    }
