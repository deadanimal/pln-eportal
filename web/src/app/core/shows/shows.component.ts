...
export class ShowsComponent implements OnInit {
  ...
  constructor(
    ...
    private dynamicService: DynamicContentsService,
  ...
  getJadual() {
    this.dynamicService.filter("category=show-schedule&status=true").subscribe(
      (res)=> {
        console.log("module shows - get jadual method(success)", res);
        this.jadualImg = res[0].description_ms;
      },
      (err)=> {
        console.log("module shows - get jadual method(error)", err);
      }
    );
