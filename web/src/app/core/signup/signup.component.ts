...
export class SignupComponent implements OnInit {
    ...
    this.registerFormGroup = this.formBuilder.group(
      {
        ...
        birth_date: ["2000-01-01", Validators.compose([])],
        gender_type: [""],
        race_type: ["ML", Validators.compose([])],
        role: [""],
  ...
  clickRegister() {
    ...
    let auth_registration_body = {
      "username": this.registerFormGroup.value.username,
      "password1": this.registerFormGroup.value.password1,
      "password2": this.registerFormGroup.value.password2,
    }

    this.authService.registerAccount(auth_registration_body).subscribe(
      (res) => {
        // console.log("res", res);
        if (res) {
          this.userService
            ...
            .subscribe(
              ...
              (err) => {
                ...
                this.toastr.error(
                  "Alamat Emel telah wujud dalam sistem.",
                  "Ralat"
                );
              }
            );
